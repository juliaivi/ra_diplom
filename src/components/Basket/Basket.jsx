...
import { basketProducts, clearbasketProducts} from "../../redux/slice/basketListSlice";
    ...
    return (
        <>
            <section className="cart">
                    ...
                    <tbody>
                        {products.map((item, index) => 
                            <tr key={index}>
                                ...
                                <td><NavLink href="/products/1.html" to={`/catalog/${item.id}`}>{item.title}</NavLink></td>
                                <td>{item.size}</td>
                                <td>{item.amount}</td>
                                <td>{item.price} руб.</td>
                                <td>{item.amount * item.price} руб.</td>
                                <td><button className="btn btn-outline-danger btn-sm" onClick={() => handelDelete(`${item.id}${item.size}`)}>Удалить</button></td>
                            </tr>
                ...
                </table>  : <><div className="empty__cart">Корзина пустая!</div></>}
            </section>
            {products.length > 0 ?
                    ...
                    <div className="card" style={{ 'maxWidth':'30rem', 'margin':'0 auto'}}>
                        <form className="card-body" onSubmit={handleSubmit}>
                            <div className="form-group">
                                ...
                                <input className="form-control" id="phone" placeholder="Ваш телефон" value={data.phone} onChange={handelChange} required/>
                            </div>
                            <div className="form-group">
                                <label htmlFor="address">Адрес доставки</label>
                                <input className="form-control" id="address" placeholder="Адрес доставки" value={data.address} onChange={handelChange} required/>
                            </div>
