...
import {
  ...
  categoriesChange,
} from "../../../redux/slice/categoriesListSlice";
import Error from "../../Error/Error";
import { Link } from "react-router-dom";

export default function Catalog() {
  let { itemCategories, categorieActive, loadingCategories, errorCategories } =
    useSelector((state) => state.categoriesList);
  let { itemCatalog, loadingCatalog, errorCatalog, itemLength, offset } =
          ...
          {itemCategories.map((categorie) => {
            return (
              <li className="nav-item" key={categorie.id}>
                <Link
                  className={
                    categorie.id === categorieActive
                      ? "nav-link active"
                      : "nav-link"
                  }
                  to="#"
                  onClick={(e) => handleChange(e, categorie.id)}
                >
                  {categorie.title}
                </Link>
              </li>
